...
def _load_validator():
    spec = importlib.util.spec_from_file_location(
        "validate_release_assets", ROOT / "tools" / "validate_release_assets.py"
    )
...
def test_release_assets_validate():
    if not list((ROOT / "tutorials").glob("*.ipynb")):
        pytest.skip("notebook not generated yet")
    validator.validate_model_card()
    validator.validate_pin_state()
    validator.validate_identity_consistency()
